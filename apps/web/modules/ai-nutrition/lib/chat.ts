"use server";

import type { ContentBlock, ToolResultBlockParam } from "@anthropic-ai/sdk/resources/messages";
import { anthropic, CLAUDE_CONFIG } from "./client";
import { ERROR_RESPONSES } from "./prompts";
import { executeFunctionCall } from "./registry";

async function startConversation(content: string) {
  return await anthropic.messages.create({
    ...CLAUDE_CONFIG,
    messages: [
      {
        role: "user",
        content,
      },
    ],
  });
}

async function processTool(content: ContentBlock) {
  let toolResult: ToolResultBlockParam = {} as ToolResultBlockParam;

  if (content.type === "tool_use") {
    const { name, input, id } = content;
    console.log(`🔧 Claude calling function: ${name}`, input);

    try {
      // Execute function using our registry
      const functionResult = await executeFunctionCall(name, input || {});
      console.log(`✅ Function ${name} result:`, functionResult);

      toolResult = {
        type: "tool_result" as const,
        tool_use_id: id,
        content: JSON.stringify(functionResult),
      };
    } catch (error) {
      console.error(`❌ Function ${name} failed:`, error);

      const errorMessage =
        error instanceof Error && error.message.includes("Unknown function")
          ? `Function ${name} is not available`
          : ERROR_RESPONSES.FUNCTION_ERROR;

      toolResult = {
        type: "tool_result" as const,
        tool_use_id: id,
        content: JSON.stringify({ error: errorMessage }),
        is_error: true,
      };
    }
  }

  return toolResult;
}

export async function chatWithNutritionAssistant(message: string) {
  try {
    console.log("🤖 Claude Processing message:", message);

    // Start conversation with Claude
    let response = await startConversation(message);

    // Handle tool calls (Claude's equivalent to function calls)
    while (response.content.some((content) => content.type === "tool_use")) {
      const toolResults = [];

      // Process each tool call
      for (const content of response.content) {
        if (content.type === "tool_use") {
          const toolResult = await processTool(content);
          toolResults.push(toolResult);
        }
      }

      // Send tool results back to Claude
      response = await anthropic.messages.create({
        ...CLAUDE_CONFIG,
        messages: [
          {
            role: "user",
            content: message,
          },
          {
            role: "assistant",
            content: response.content,
          },
          {
            role: "user",
            content: toolResults,
          },
        ],
      });
    }

    // Extract text response
    const textContent = response.content.find(
      (content): content is Extract<ContentBlock, { type: "text" }> => content.type === "text"
    );
    const responseText = textContent?.text || "Sorry, I couldn't generate a response.";

    return {
      success: true,
      message: responseText,
      usage: response.usage,
    };
  } catch (error) {
    console.error("💥 Claude chat error:", error);

    // Enhanced error handling
    if (error instanceof Error) {
      // API Key errors
      if (error.message.includes("API key") || error.message.includes("authentication")) {
        return {
          success: false,
          message: "🔑 AI service configuration issue. Please contact support.",
          error: "API_KEY_ERROR",
        };
      }

      // Rate limit errors
      if (error.message.includes("rate limit") || error.message.includes("quota")) {
        return {
          success: false,
          message: ERROR_RESPONSES.API_LIMIT,
          error: "RATE_LIMIT_ERROR",
        };
      }

      // Network errors
      if (error.message.includes("fetch") || error.message.includes("network")) {
        return {
          success: false,
          message: "🌐 Network connection issue. Please check your internet and try again.",
          error: "NETWORK_ERROR",
        };
      }
    }

    // Default error
    return {
      success: false,
      message: ERROR_RESPONSES.GENERAL_ERROR,
      error: error instanceof Error ? error.message : "Unknown error",
    };
  }
}
